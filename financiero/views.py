...
from acceso.models import Corte, CorteApoyo
from acceso.forms import CorteForm, CorteApoyoForm
from .forms import LiquidacionGestorForm, LiquidacionFormadorForm
from acceso.models import Evidencia,Actividad, EvidenciaApoyo
...
class GestorTipoApoyoView(FinancieroMixin,TemplateView):
    template_name = 'listado_gestores_financiero_apoyo.html'

    def get_context_data(self, **kwargs):
        kwargs['REGION'] = Region.objects.get(pk=self.kwargs['pk']).nombre
        kwargs['ID_REGION'] = self.kwargs['pk']
        kwargs['ID_TIPO'] = 2
        return super(GestorTipoApoyoView,self).get_context_data(**kwargs)
...
class GestorApoyoCorteEvidenciaView(FinancieroMixin,TemplateView):
    template_name = 'listado_gestores_evidencia_corte_apoyo.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs['ID_CORTE'] = self.kwargs['corte_id']
        kwargs['ID_GESTOR'] = self.kwargs['gestor_id']
        kwargs['CORTE'] = CorteApoyo.objects.get(pk=self.kwargs['corte_id']).titulo
        kwargs['GESTOR'] = Gestor.objects.get(pk=self.kwargs['gestor_id']).nombre
        return super(GestorApoyoCorteEvidenciaView,self).get_context_data(**kwargs)
...
class NuevoGestorApoyoView(FinancieroMixin, CreateView):
    model = Gestor
    form_class = NuevoForm
    success_url = "../"
    template_name = "nuevo_gestor.html"
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs['ID_REGION'] = self.kwargs['pk']
        kwargs['ID_TIPO'] = 2
        return super(NuevoGestorApoyoView,self).get_context_data(**kwargs)
...
class NuevoCorteApoyoView(FinancieroMixin, CreateView):
    model = CorteApoyo
    form_class = CorteApoyoForm
    success_url = "../"
    template_name = "nuevo_corte.html"

    def get_context_data(self, **kwargs):
        reporte = EvidenciaApoyo.objects.filter(radicado__region__id=self.kwargs['pk']).filter(corte=None).exclude(soporte="")
        kwargs['CANTIDAD'] = reporte.count()
        kwargs['VALOR'] = "$ "+str(reporte.aggregate(Sum('valor__valor'))['valor__valor__sum'])
        kwargs['REGION'] = Region.objects.get(pk=self.kwargs['pk']).nombre
        kwargs['ID_REGION'] = self.kwargs['pk']

        return super(NuevoCorteApoyoView,self).get_context_data(**kwargs)
...
def reporte_quincenal_financiero(request,pk):
    ...
    gestores = Gestor.objects.filter(region__id=pk).filter(tipo__id=1)
...
def reporte_quincenal_financiero_apoyo(request,pk):
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    response['Content-Disposition'] = 'attachment; filename=Reporte de Quincenas.xlsx'
    archivo = openpyxl.load_workbook(settings.STATICFILES_DIRS[0]+'/formatos/base.xlsx')
    ...
    cortes = CorteApoyo.objects.filter(region__id=pk)
    ...
    gestores = Gestor.objects.filter(region__id=pk).filter(tipo__id=2)
    ...
    for gestor in gestores:
        ...
        for corte in cortes:
            row.append(EvidenciaApoyo.objects.filter(gestor__id=gestor.id).exclude(corte=None).filter(corte=corte).aggregate(Sum('valor__valor'))['valor__valor__sum'])
...
def reporte_gestor_apoyo(request,pk,corte_id,gestor_id):
    tipo_id = 2
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    gestor = Gestor.objects.get(id=gestor_id)
    corte = Corte.objects.get(id=corte_id)
...
def reporte_gestor_email_apoyo(request,pk,corte_id,gestor_id):
    tipo_id=2
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    gestor = Gestor.objects.get(id=gestor_id)
    corte = Corte.objects.get(id=corte_id)
